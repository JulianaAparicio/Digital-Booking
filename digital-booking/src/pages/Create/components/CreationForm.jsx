...
const dummyAmenities = ['wifi', 'tv', 'smoke', 'pool', 'pets', 'party', 'parking'];

const CreationForm = ({ isValid }) => {
   const context = useContext(Context);
   const states = useContext(StateContext);

   const filterAmenities = () =>
      dummyAmenities.filter(am => !states.amenities.state[0].some(am2 => am2 === am));
   ...
   useEffect(() => {
      setAmenitySelected(visibleAmenities[0]);
   }, [visibleAmenities]);
            ...
            <Card>
               ...
               <article className="db-product-amenities">
                  ...
                  <div className="select-container">
                     ...
                     <div className="amenities-selected">
                        {states.amenities.state[0].map((amenity, i) => (
                           <div key={i} onClick={() => deleteAmenity(i)} data-index={i}>
                              <Amenity data-index={i} type={amenity} label={amenity} />
                           </div>
                        ))}
                     </div>
                  </div>
               ...
               <article className="db-product-highlights">
                  ...
                  <div className="inputs">
                     <Input
                        type="textarea"
                        setValue={e => states.rules.state[1](e.split('-').map(string => string))}
                        label={'Normas de la casa'}
                        placeholder="Separe cada indicación con un guión bajo ( _ )"
                     />
                     <Input
                        type="textarea"
                        setValue={e => states.security.state[1](e.split('_').map(string => string))}
               ...
               <article className="db-product-images">
                  ...
                  <div className="images-container">
                     {states.images.state[0].map((a, i) => (
                        <div data-index={i} onClick={deleteImage} className="image-preview" key={i}>
                           <Card>
               ...
               <article className="db-button-container">
                  <Button isDisabled={!isValid} classList="db-button-primary">
                     Crear
                  </Button>
               </article>
            </Card>
